...
module.exports = {
  ...
  run: async (client, serverInfo, sql) => {
    sql
      .all(`select * from Members where MutedUntil is not null`)
      .then(rows => {
        rows.forEach(row => {
          if (row.MutedUntil < new Date().getTime()) {
            let MutedRole = client.guilds
              .get(serverInfo.guildId)
              .roles.find("name", "Muted");
            let MutedUser = client.guilds
              .get(serverInfo.guildId)
              .members.get(row.DiscordID);
    ...
    sql
      .get(`select Value from CurrentStats where Type = 'toponline'`)
      .then(row => {
        var oldVal = row.Value;
        var newVal = client.guilds
          .get(serverInfo.guildId)
        ...
        if (oldVal == undefined || oldVal == null || oldVal < newVal) {
          sql
            .run(
              `Update CurrentStats set Value = '${newVal}', Time = '${new Date().getTime()}' where Type = 'toponline'`
            )
