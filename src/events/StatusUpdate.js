            ...
            if (rows.length == 0) {
                ...
                if (rows[rows.length - 1].Active == 1) {
                    sql.run(`update Statuses set Active = 0 where ID = ${rows[rows.length - 1].ID}`).then(() => {
                        ...
                        client.user.setActivity(rows[0].StatusText.replace("counter", client.guilds.get(serverInfo.guildId).memberCount), {type: rows[0].StatusType, url: "https://www.twitch.tv/alphaconsole"});
                    ...
                    for (let i = 0; i < rows.length; i++) {
                        if (rows[i].Active == 1) {
                            sql.run(`update Statuses set Active = 0 where ID = ${rows[i].ID}`).then(() => {
                                ...
                                client.user.setActivity(rows[i + 1].StatusText.replace("counter", client.guilds.get(serverInfo.guildId).memberCount), {type: rows[i + 1].StatusType, url: "https://www.twitch.tv/alphaconsole"});
