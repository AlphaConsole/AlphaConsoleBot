...
module.exports = {
    ...
    run: async(client, serverInfo, message ,args) => {
        if (hasRole(message.member, "Moderator") || hasRole(message.member, "Admin") || hasRole(message.member, "Developer")) {
            ...
            if (args.length == 1) {
                amount = 2;
                message.channel.messages.fetch({limit: amount}).then(messages => message.channel.bulkDelete(messages)).catch(console.error);
                ...
                if(!isNumber(args[1]) || args[1].startsWith(".")) {
                    const embed = new Discord.MessageEmbed()
                    .setColor([255,255,0])
                    .setAuthor('The command has not correctly been used. Please use `!clear [amount]`', serverInfo.logo) 
