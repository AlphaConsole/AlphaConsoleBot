...
module.exports = {
    ...
    run: ({ client, serverInfo, message, args, sql, config, sendEmbed }) => {
        ...
        try {
            ...
            sql.query("Select * from Titles where DiscordID = ?", [ id ], (err, rows) => {
                if (err) return console.error(err);
                sql.query("Select * from Players where DiscordID = ?", [ id ], (err, steams) => {
                    if (err) return console.error(err);
                    const user = rows[0];
