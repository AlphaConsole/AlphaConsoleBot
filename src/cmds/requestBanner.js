...
module.exports = {
  ...
  run: (
    ...
    if (!override) {
      if (message.channel.id !== serverInfo.channels.setBanner) return;

      try {
        let url;
        if (message.attachments.first()) url = message.attachments.first().url;
        else if (ValidURL(args[2])) url = args[2];
        else {
          sql.query(
            "SELECT * FROM Banners WHERE Name = ? OR ID = ?",
            [args[2], args[2]],
            (err, res) => {
              if (res[0]) {
                sql.query("UPDATE Titles SET Banner = ? WHERE DiscordID = ?", [
                  res[0].Path,
                  message.author.id
                ]);
                message.author.send("Your banner has been set to:", {
                  files: [
                    config.keys.cdn_banners +
                      "thumbnail/" +
                      res[0].Path.split(" ").join("%20") +
                      ".png"
                  ]
                });
              }
        ...
        sql.query(
          ...
          [message.author.id],
          (err, res) => {
            if (err) return message.delete().catch(e => {});

            const user = res[0];

            if (!user) {
              message.author.send(
                "Hi, in order to use our custom title service (and thereby also the banners) you must authorize your discord account. \n" +
            ...
            jimp.read(url.split(" ").join("%20"), (err, image) => {
              ...
              if (message.member.isModerator) {
                  ...
                  (err, res) => {
                    ...
                    request(urlCall, function(err, res, body) {
                      ...
                      config.sql.query(
                        ...
                        [body.trim(), message.author.id],
                        err => {
                          if (err) return console.error(err);

                          sendEmbed(
                            message.author,
                            "Your banner has been applied",
                            undefined,
                            undefined,
                            url
                          );
                ...
                return client.channels
                  .get(serverInfo.channels.banners)
                  .send(`**New Banner Request**\nUser:${message.author}`, {
                    files: [imgPath]
                  })
                  .then(async m => {
                    ...
                    sendEmbed(
                      message.author,
                      "Banner request sent. Please be patient.",
                      undefined,
                      undefined,
    ...
    let url;
    if (message.attachments.first()) url = message.attachments.first().url;
    else if (ValidURL(args[3])) url = args[3];
    else {
      sql.query(
        "SELECT * FROM Banners WHERE Name = ? OR ID = ?",
        [args[3], args[3]],
        (err, res) => {
          if (res[0]) {
            ...
            message.author.send(`<@${id}>'s banner has been updated to:`, {
              files: [
                config.keys.cdn_banners.split(" ").join("%20") +
                  "thumbnail/" +
                  res[0].Path +
                  ".png"
              ]
