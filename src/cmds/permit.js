...
module.exports = {
    ...
    run: async(client, serverInfo, sql, message ,args, permits) => {
        if (hasRole(message.member, "Moderator") || hasRole(message.member, "Admin") || hasRole(message.member, "Developer")) {
            ...
            .setColor([255,255,0])
            .setAuthor(`${client.users.get(TheUser).tag} is now permitted to post links for 5 minutes.`, serverInfo.logo) 
            message.channel.send(embed)

            const embedlog = new Discord.MessageEmbed()
            .setColor([255,255,0])
            .setAuthor('Link Permit', serverInfo.logo)
            .addField("The user", `**${client.users.get(TheUser).tag}** (<@${TheUser}>)`)
            .addField(`has been permitted at`, `**${message.channel.name}** (${message.channel})`)
