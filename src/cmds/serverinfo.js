...
module.exports = {
  ...
  run: async (client, serverInfo, sql, message, args) => {
    if (
      ...
      if (!guild.region || guild.region.length < 1)
        console.log("Region failed");
      if (
        !guild.channels.array().filter(c => c.type.toLowerCase() == "text")
          .length
      )
        console.log("Text Channels failed");
      if (
        !guild.channels.array().filter(c => c.type.toLowerCase() == "voice")
          .length
      )
        console.log("voice Channels failed");
      if (!guild.roles.size) console.log("Roles failed");
      if (!guild.owner.user.tag || guild.owner.user.tag.length < 1)
        console.log("Owner failed");
      if (
        !guild.emojis.map(m => m.toString()).join(" ") ||
        guild.emojis.map(m => m.toString()).join(" ").length < 1
      )
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .addField(
          ...
          `${
            guild.members
              .array()
              .filter(m => m.presence.status.toLowerCase() !== "offline").length
          }/${guild.memberCount}`,
          true
        )
        .addField(
