    ...
    run: async(client, serverInfo, message, args, Commands) => {
        if (args.length == 1) {
            ...
            for (var key in Commands) {
                ...
                if (Commands[key].perms.toLowerCase() == "staff") staff += Commands[key].title + "\n";
                if (Commands[key].perms.toLowerCase() == "support") support += Commands[key].title + "\n";
                if (Commands[key].perms.toLowerCase() == "moderator") moderator += Commands[key].title + "\n";
                if (Commands[key].perms.toLowerCase() == "admin") admin += Commands[key].title + "\n";
            }
            ...
            if(Commands[args[1].toLowerCase()] != undefined) {
                ...
                .setFooter("Permission Level: " + Commands[args[1].toLowerCase()].perms)

                for(var key in Commands[args[1].toLowerCase()].commands) {
                    embed.addField(Commands[args[1].toLowerCase()].commands[key], Commands[args[1].toLowerCase()].desc[key])
                }
                message.channel.send(embed)
            } else {
                const embed = new Discord.MessageEmbed()
