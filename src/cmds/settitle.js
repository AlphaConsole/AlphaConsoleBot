...
module.exports = {
  ...
  run: ({ client, serverInfo, message, args, sql, config, sendEmbed, member }) => {
     ...
     */
    function setTitle() {
      if (args.length < 3) return sendEmbed(message.author, "You must have forgotten the title itself ;)");

      let userTitle = createTitle(args, 2);
      setUsersTitle(message.author.id, userTitle)
      .then(title => sendEmbed(message.author, "Title updated", `Your title has been set to **\`${title}\`**!`))
     ...
     */
    function setColor() {
      if (args.length < 3) return sendEmbed(message.author, "You must have forgotten the color itself ;)");
    ...
    function overrideColor() {
      ...
      let id = message.mentions.users.first() ? message.mentions.users.first().id : args[2];
      let color = createTitle(args, 3).replace(/ /g,'').toUpperCase();
     ...
     */
    function setGlow() {
      if (args.length < 3) return sendEmbed(message.author, "You must have forgotten the glow itself ;)");
        ...
        if (!overridingUser && !message.member.isAdmin) {
          sql.query("Select * from TitleReports where DiscordID = ? AND Title = ? AND Permitted = 1", [ id.trim(), title ], (err, res) => {
            ...
            sql.query("Select * from Config where Config = 'blacklistedWords'", [], (err, rows) => {
              ...
              let blacklistedTitles = titles.filter(t => thisBlacklist.find(b => b && typeof b === "string" && t.toLowerCase().includes(b.toLowerCase())));
