...
module.exports = {
  ...
  run: async (client, serverInfo, message, args, sql) => {
    if (isStaff(message.member)) {
      var giveawayChannel = client.guilds
        .get(serverInfo.guildId)
        .channels.get(serverInfo.giveawaychannel);

      await sql
        .get(`select * from misc where function = 'giveawaymsg'`)
        .then(row => {
          if (row) {
            var msgId = row.value;
            ...
            giveawayChannel.messages.fetch(msgId).then(msg => {
              msg.reactions
                ...
                .then(usersRaw => {
                  ...
                  sql.run(
                    `update misc set value = ? where function = 'giveawaywinners'`,
      ...
      await client.guilds
        .get(serverInfo.guildId)
        .roles.get(serverInfo.giveawayrole)
        .setMentionable(true)
        .then(async r => {
          ...
          await giveawayChannel.send("", {
            files: [
              "https://cdn.discordapp.com/attachments/389241234100715520/415325374369759252/ACGiveaways.png"
            ]
          });
          await giveawayChannel.send("", {
            files: [
          ...
          sql
            .get(`select * from misc where function = 'giveawaywinners'`)
            .then(async row => {
              ...
              sql
                .get(`select * from misc where function = 'giveawayon'`)
                .then(async legacyon => {
                  if (legacyon.value == 0) {
                    await giveawayChannel.send(
                    ...
                    await giveawayChannel.send(
                      "‌\n\nDo `!role ga` in <#" +
                        serverInfo.BotSpam +
                        "> to stay notified on new giveaways."
                    );
                    await giveawayChannel.send("", {
                      files: [
                        "https://cdn.discordapp.com/attachments/381877174610821130/395685201562435584/ACBorder.png"
                    ...
                    await giveawayChannel.send(embed).then(m => {
                      ...
                      sql
                        .get(
                          `select * from misc where function = 'giveawaymsg'`
                        )
                        .then(async row => {
                          if (!row) {
                            await sql.run(
                              `insert into misc(function) VALUES('giveawaymsg')`
                            );
                          }
                          await sql.run(
                            `update misc set value = '${
                              m.id
                            }' where function = 'giveawaymsg'`
                          );
                        });
                    });
