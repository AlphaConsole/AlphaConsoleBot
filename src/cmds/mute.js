...
module.exports = {
  ...
  details: [
    {
      perms: "Support",
      command:
        "!Mute <@tag | user Id> <Length[TimeUnit(d,h,m,s) - default: h]> <?Reason>",
      description:
        "Mutes the person for the given amount of times. If time is 0 then the person is permanently muted"
    }
  ],
  ...
  run: ({ client, serverInfo, message, args, sql, config, sendEmbed }) => {
    ...
    if (args.length < 3)
      return sendEmbed(
        message.channel,
        "You must have forgotten the user or the time",
    ...
    client.guilds
      ...
      .then(m => {
        ...
        require("../helpers/checkUser").run(sql, m.user, async (err, user) => {
          ...
          sql.query(
            "Insert into `Logs`(Action, Member, Moderator, value, Reason, Time, ChannelID) values(?, ?, ?, ?, ?, ?, ?)",
            [
              "mute",
              m.id,
              message.author.id,
            ...
            (err, res) => {
              ...
              const embedlog = new Discord.MessageEmbed()
                .setColor([255, 255, 0])
                .setAuthor(
                  `Case ${caseId} | User Mute`,
                  client.user.displayAvatarURL({ format: "png" })
                )
                .setDescription(
                  timeArg == 0
                    ? `${m} (${m.id}) has been permanently muted by ${
                        message.member
                      }`
                    : `${m} (${
                        m.id
